...
def save_cached_session():
    cache = {
        ...
        "answers_history": st.session_state.get("answers_history", {}),
    }
...
def undo_last_grade():
    if st.session_state.current_index > 0:
        ...
        # Restore the answers the user actually gave on that card, so the
        # solution screen shows their real pinyin attempt and MCQ verdict
        # instead of blanks.
        prev_answers = st.session_state.get("answers_history", {}).get(idx_str, {})
        st.session_state.user_pinyin = prev_answers.get("user_pinyin", "")
        st.session_state.mcq_correct = prev_answers.get("mcq_correct")
...
if current_mode == 'listen':
    ...
    if st.session_state.stage == 1:
        # Key is per-card: with a shared key, the previous card's answer
        # stayed pre-filled in the box on every subsequent card.
        pinyin_key = f"pinyin_input_{st.session_state.current_index}"
        st.text_input("Type the Pinyin you hear:", key=pinyin_key)
        if st.button("Submit Pinyin", type="primary", use_container_width=True):
            st.session_state.user_pinyin = st.session_state.get(pinyin_key, "")
            idx_str = str(st.session_state.current_index)
            st.session_state.setdefault("answers_history", {}).setdefault(idx_str, {})[
                "user_pinyin"] = st.session_state.user_pinyin
            advance_to_stage(2)
            st.rerun()
